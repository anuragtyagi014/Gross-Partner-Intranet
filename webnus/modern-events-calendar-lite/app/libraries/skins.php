    ...
    public function sf_search_field($field, $options, $display_label = null)
    {
        ...
        if($field == 'category')
        {
            ...
            elseif($type == 'simple-checkboxes' and wp_count_terms(array('taxonomy' => 'mec_category')))
            {
                ...
                {
					$term_cat_color = get_term_meta($term_categroy->term_id, 'mec_cat_color', true);
                    $output .= '<li id="mec_category-'.esc_attr($term_categroy->term_id).'">
                        <label class="selectit" style="background-color:'.$term_cat_color.'"><input value="'.esc_attr($term_categroy->term_id).'" title="'.esc_attr($term_categroy->name).'" type="checkbox" name="tax_input[mec_category][]" id="in-mec_category-'.esc_attr($term_categroy->term_id).'" '.(in_array($term_categroy->term_id, $selected) ? 'checked' : '').'> '.esc_html($term_categroy->name).'</label>
                    </li>';
                }
