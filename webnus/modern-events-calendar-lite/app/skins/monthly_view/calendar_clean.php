    ...
    <?php
        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                $events_str .= '<div class="mec-calendar-events-sec has-events" id="mec-calendar-events-sec-'.esc_attr($this->id.'-'.$day_id).'" data-mec-cell="'.esc_attr($day_id).'" '.((trim($selected_day) != '' or $this->display_all) ? ' style="display: block;"' : '').'>'.MEC_kses::element($this->day_label($time));
            ...
            else
            {
                ...
                if(!$this->display_all)
                {
                    $events_str .= '<div class="mec-calendar-events-sec" id="mec-calendar-events-sec-'.esc_attr($this->id.'-'.$day_id).'" data-mec-cell="'.esc_attr($day_id).'" '.((trim($selected_day) != '' or $this->display_all) ? ' style="display: block;"' : '').'><div class="day-name-date"><h3 class="day-name">'.__(date('l',$time)).'</h3><h4 class="day-date">'.date('d.',$time).'</h4></div>';
