        ...
        for($list_day = 1; $list_day <= $days_in_month; $list_day++)
        {
            ...
            if(isset($events[$today]) and count($events[$today]))
            {
                ...
                foreach($events[$today] as $event)
                {
					...
					$first_cat_color = '';
					if(is_array($event->data->categories)){
						foreach($event->data->categories as $category){
							if($category['color'] != ''){
								$first_cat_color = $category['color'];
								break;
							}
						}
					}
					
                    echo '<div style="background:'.$first_cat_color.'" class="mec-single-event-novel mec-event-article mec-event-id-'.$event->data->ID.' '.$this->get_event_classes($event).'">'; // $event_color
					echo '<style type="text/css">.mec-event-article.mec-event-id-'.$event->data->ID.':after { background-color:'.$first_cat_color.' !important; }</style>';
					...
					$event_categories = '';
					$event_time = '';
					$event_start_date = $event->data->meta['mec_date']['start']['date'].' '.sprintf('%02d', $event->data->meta['mec_date']['start']['hour']).':'.sprintf('%02d', $event->data->meta['mec_date']['start']['minutes']).' '.$event->data->meta['mec_date']['start']['ampm'];
					$event_start_timestamp = strtotime($event_start_date);
					if(is_array($event->data->locations)){
						foreach($event->data->locations as $location){
							if($event_locations != ''){
								$event_locations .= ', ';
							}
							$event_locations .= $location['name'];
						}
					}
					if(is_array($event->data->categories)){
						foreach($event->data->categories as $category){
							$event_categories .= '<span class="cat" style="background-color:'.$category['color'].'">'.$category['name'].'</span>';
						}
					}
					$event_time = date('H:i',$event_start_timestamp).' '.__('Uhr', 'gross-partner-intranet');
					if($event->data->meta['mec_date']['allday'] == '1'){
						$event_time = __('ganztags', 'gross-partner-intranet');
					}

					$event_video = get_field('video',$event->data->ID);

                    echo $this->display_cost($event);
                    echo $this->display_custom_data($event);
					echo '<div class="tooltip">';
					echo '<div class="category date">'.$event_categories.'<span class="date">'.date('d.m.Y',$event_start_timestamp).'</span></div>';
					echo '<div class="title"><p>'.$event->data->title.'</p></div>';
					echo '<div class="time"><p>'.$event_time.'</p></div>';
					echo '<div class="location"><p>'.$event_locations.'</p></div>';
					if(isset($event_video['ID']) && $event_video['ID'] > 0){
						echo '<div class="video">';
						echo '<video src="'.$event_video['url'].'"'.(isset($event->data->featured_image['medium']) ? ' poster="'.$event->data->featured_image['medium'].'"' : '').' loop muted playsinline>';
						echo '</div>';
					}elseif(isset($event->data->featured_image['medium'])){
						echo '<div class="image">';
						echo '<img src="'.$event->data->featured_image['medium'].'" alt="">';
						echo '</div>';
					}
					echo '</div>';
                    echo '</div>';
