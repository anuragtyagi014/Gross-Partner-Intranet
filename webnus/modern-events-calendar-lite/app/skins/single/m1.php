...
<div class="mec-wrap <?php echo esc_attr($event_colorskin); ?> clearfix <?php echo esc_attr($this->html_class); ?> mec-modal-wrap" id="mec_skin_<?php echo esc_attr($this->uniqueid); ?>">
    <article class="mec-single-event mec-single-modern mec-single-modal">
		...
		<?php
		$event_image = $event->data->featured_image;
		...
		if(isset($event_video['ID']) && $event_video['ID'] > 0){
			...
			echo '<video src="'.$event_video['url'].'"'.(isset($event_image['large']) ? ' poster="'.$event_image['large'].'"' : '').' autoplay controls loop muted playsinline>';
			echo '</div>';
			echo '</div>';
		}elseif(isset($event_image['large'])){
			echo '<div class="mec-events-event-image">';
			// echo MEC_kses::element($this->display_image_module($event));
			echo '<div class="image">';
			echo '<img src="'.$event_image['large'].'" alt="">';
			echo '</div>';
			echo '</div>';
		}
		?>
                ...
                if(isset($event->data->categories) and !empty($event->data->categories))
                {
                    ...
                    <div class="mec-single-event-category">
                        ...
                        foreach($event->data->categories as $category)
                        {
                            ...
                            echo '<dd class="mec-events-event-categories"><a class="mec-color-hover" rel="tag">' . MEC_kses::element($icon . esc_html($category['name']) . $color_html) .'</a></dd>'; // href="'.esc_url(get_term_link($category['id'], 'mec_category')).'"
                        }
                        ?>
                        </dl>
                    </div>
